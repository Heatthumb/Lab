...
app = Flask(__name__)
app.secret_key = "studio_v55_hover_patched"
...
<head>
    ...
    <style>
        ...
        .canvas-container:hover { transform: scale(1.1); z-index: 100; box-shadow: 0 20px 50px rgba(0,0,0,0.8); cursor: crosshair; }

        .canvas-area { position: relative; width: 100%; height: 100%; background: #000; overflow: hidden; border-radius: 8px; border: 0px solid transparent; }
...
<body>
    ...
    <div style="display:flex; height:100vh; width:100vw; align-items:center; justify-content:center;">
        <form method="POST" action="/login" style="background:var(--card); padding:40px; border-radius:12px; border:1px solid var(--border);">
            <h2 style="color:var(--mint); margin-top:0;">Viral Studio V55</h2>
    ...
    <script>
        ...
        function exportFrame(i, format) {
            if(!format) return;
            html2canvas(document.getElementById(`export-${i}`), { useCORS: true, scale: 3 }).then(canvas => {
                const link = document.createElement('a');
                link.download = `Viral_V55_${i}.${format}`;
